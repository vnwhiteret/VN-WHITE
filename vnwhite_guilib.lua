...
local function initFonts()
    ...
    print("[vnwhite font: " .. tostring(picked))
end
...
local function fontInitCoro()
    ...
    if gdi32 then
        ...
        print("[vnwhite ffi/gdi32 unavailable, using system fonts")
    end
...
function Section:_widget(wd, x, y, w)
    ...
    elseif wd.kind == "button" then
        ...
        if clicked(x, y + 1, w, bh) then
            local ok, err = pcall(wd.cb); if not ok then print("[vnwhite button error: " .. tostring(err)) end
        end
    ...
    elseif wd.kind == "custom" then
        if wd.fn then
            ...
            if not ok then print("[vnwhite custom widget error: " .. tostring(err)) end
            local used = UI._cy - y
...
local function renderSectionAt(s, x, y, w)
    ...
    if not ok then print("[vnwhite section '" .. tostring(s.title) .. "' error: " .. tostring(err)); return h end
    return rh
end
...
function M:_drawWatermark()
    ...
    local segs = {}
    if wm.parts.cheat then segs[#segs + 1] = nameSeg(wm.cheat_name or "GAMESENSE.PUB") end
    if wm.parts.lua   then segs[#segs + 1] = nameSeg(wm.lua_name or "VNWHITE.CC") end
...
function M:_frame()
    ...
    if tab then
        ...
        if not ok then print("[vnwhite tab '" .. tostring(tab.name) .. "' error: " .. tostring(err)) end
    end
...
function M:Build(opts)
    ...
    if not _getMouse then print("[vnwhite WARNING: mouse position API not found -- cursor won't track") end
    ...
    callbacks.Register("Draw", function()
        ...
        if not ok then print("[vnwhite frame error: " .. tostring(err)) end
    end)
    ...
    print(string.format("[vnwhite.cc] guilib v%s ready: %d tabs, mouse=%s clock=%s",
